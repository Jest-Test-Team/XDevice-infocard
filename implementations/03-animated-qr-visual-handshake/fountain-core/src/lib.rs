...
mod tests {
    ...
    fn decode_accepts_identical_duplicate_symbols() {
        let symbols = vec![
            Symbol::new(9, 0, 3, b"foo".to_vec()),
            Symbol::new(9, 0, 3, b"foo".to_vec()),
            Symbol::new(9, 1, 3, b"bar".to_vec()),
            Symbol::new(9, 2, 3, b"\x04\x0e\x1d".to_vec()),
        ];
        let decoded = decode(&symbols).expect("decode should succeed");
    ...
    fn decode_rejects_conflicting_duplicate_symbols() {
        let symbols = vec![
            Symbol::new(11, 0, 3, b"foo".to_vec()),
            Symbol::new(11, 0, 3, b"zzz".to_vec()),
            Symbol::new(11, 1, 3, b"bar".to_vec()),
            Symbol::new(11, 2, 3, b"\x04\x0e\x1d".to_vec()),
        ];
        let err = decode(&symbols).expect_err("decode should fail");
